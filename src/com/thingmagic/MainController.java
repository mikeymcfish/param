...
public class MainController implements Initializable {
    ...
    public MainController() {
        ...
        Platform.runLater(new Runnable(){
            public void run(){
                MainController._instance = that;
                System.out.println("Singleton Set");
            }
        });
    }
    ...
    public static MainController Instance() {
        if (Platform.isFxApplicationThread()){
            System.out.println("NOT IN THE FX APPLICATION THREAD.");
        }
        return _instance;
    }
    
    public static void doMessage(String t, String m){
       Platform.runLater(new Runnable(){
            public void run(){
                if (Platform.isFxApplicationThread()){
                    System.out.println("NOT IN THE FX APPLICATION THREAD FROM doMessage Call.");
                }
                else {
                    System.out.println("I AM IN THE THREAD IN DO MESSAGE");
                }
                MainController.Instance().showWarningErrorMessage(t, m);
            }
        }); 
    }
    
    //Api call variables
    //debug root: http://192.168.1.139:8888/api/
    //live root: http://hq.pixelacademy.org/api/ 
    public static final String API_ROOT = "http://hq.pixelacademy.org/api/";
    public static final String API_CHECKIN_URL = "rfid";
    public static final String API_ASSIGN_URL = "rfid/";
    public static final String API_UPDATE_ROSTER_URL = "checkin_roster/3";//always brooklyn
    public static final String API_CLEAR_EPC_URL = "epc_clear/";
    public static final String API_SECRET_KEY = "pixelacademy_token";
    
    //Bracelet Assign variables
    public static int assignMemberID = 0;
    public static boolean assignMode = false;
    public static String assignEPC = "";
    
    public static void resetAssignMode() {
        MainController.assignEPC = "";
        MainController.assignMode = false;
        MainController.assignMemberID = 0;
    }
    
    public static void setAssignMode(int mid){
        MainController.assignMode = true;
        MainController.assignMemberID = mid;
    }
    ...
    @FXML
    private TableColumn memberAssignColumn;
     @FXML
    private TableColumn memberEpcColumn;
    @FXML 
    private Text uniqueMemberCount;
    private ObservableList<MemberResult> assign_row;
    
    @FXML
    private TableView assignEpcTableView;
    @FXML
    private TableColumn assignEpcColumn;
    private ObservableList<TagResults> assign_epc_row;
...
//                            }
        ...
        {            
           ...
           try
           {
            ...
            while (portList.hasMoreElements())
            {
                ...
                {
                    if(true)//!(portId.getName().contains("Bluetooth") ||  portId.getName().contains("tty.usbserial")))
                    {
           ...
           {
               System.out.println("Error getting devices:");
               System.out.println(e.getClass() + ": " + e.getMessage());
               StackTraceElement[] ste = e.getStackTrace();
               System.out.println("Stack trace:");
               for(int i = 0; i < ste.length; i++){
                   System.out.println("Line: " + ste[i].getLineNumber() + "@" + ste[i].getMethodName() );
               }
           }
           catch(Exception ex)
           {
               System.out.println("Exception getting devices:");
    ...
    private void connect(ActionEvent event) 
    {
        ...
        {          
            ...
            {
                new Thread(dissConnectThread).start();
            }
        }
        catch (Exception e)
        {
        ...
        Platform.runLater(new Runnable()
        {
            ...
            public void run()
            {
                ...
                { 
                    ...
                    setTitledPanesStatus(true, true, true, true, false, true);
                    setTitledPanesExpandedStatus(false, false, false, false, true, false);
                }
                ...
                public void run()
                {
                    try
                    {
                        ...
                        if (isConnected)
                        {
                            ...
                            connectionButton.setText("Disconnect");
                            connectionButton.setDisable(false);
                            setTitledPanesStatus(false, false, false, false, false, false);
                            setTitledPanesExpandedStatus(true, false, false, false, false, false);
    ...
    private Runnable dissConnectThread = new Runnable()
    {
        @Override
        public void run() 
        {
    ...
    private void findAntennas(ActionEvent event)
    {
        try
        {
            ...
            {
                antennaDetection.setSelected(false);
                showWarningErrorMessage("warning", "Plese select the region");
            }
        }
...
//    }
    ...
    {
        hideMessagePopup();
        String message = "Now Applying selected configurations on module.";
    ...
    {
       // System.out.println("Starting config apply");
        if (isConnected) 
        {
            ...
            try
            {
               ...
               //System.out.println("line 2692");
               //System.out.println(autonomousRead.isSelected());
               //System.out.println(gpiTriggerRead.isSelected());   
                if (autonomousRead.isSelected() || gpiTriggerRead.isSelected())
                {
                    ...
                    try
                    {
                        //System.out.println("trying some user config stuff");
                        ...
                        setTitledPanesStatus(true, true, false, true, true, false);
                        setTitledPanesExpandedStatus(false, false, true, false, false, true);
                        loadFirmware.disableProperty().setValue(true);
                        loadFirmware.setOpacity(disableOpacity);
                        
                        loadConfigButton.disableProperty().setValue(true);
                        loadConfigButton.setOpacity(disableOpacity);
                        
                        saveConfigButton.disableProperty().setValue(true);
                        saveConfigButton.setOpacity(disableOpacity);
                        ...
                        //System.out.println("Starting data post thread");
    ...
    public void setTitledPanesStatus(boolean rw, boolean pt, boolean dp, boolean pf, boolean fu, boolean abt)
    {
        Platform.runLater(new Runnable() 
        {
            @Override
    ...
    public void setTitledPanesExpandedStatus(boolean rw, boolean pt, boolean dp, boolean pf, boolean fu, boolean abt)
    {
        Platform.runLater(new Runnable() 
        {
            @Override
        ...
        if(loadSaveProperties != null)
        {
            ...
            for(Object key: prop.keySet())
            {
                   ...
                   try
                   {
                      ...
                      else
                      {
                         ...
                         loadSaveError.append("Read power "+ param +" value "+ value
                               + " setting failed.  Please enter with in "+minReaderPower+" and  "+maxReaderPower
                ...
                else if(param.equalsIgnoreCase("/reader/gen2/tagEncoding"))
                {
                    ...
                    gen2TagEncodingChangeConfiguration();
                }
...
//                           addChangeList("Embedded End previous value: " + changeListMap.get("embeddedEnd") + " Now: " + embeddedEnd.getText());
    ...
    private Runnable dataPostThread = new Runnable()
    {
        ...
        public void run()
        {
            try
            {
                //System.out.println("Data Post Thread");
                while(isReading)
                {
                    //System.out.println("Reading");
                    ...
                    while (iterator.hasNext())
                    {
                        ...
                        if(embeddedReadEnable.isSelected())
                        {
                            //System.out.println("embedded read");
                        ...
                        {
                            //System.out.println("non embedded read");
                    ...
                    Platform.runLater(new Runnable()
                    {
                        ...
                        {  
                            ...
                            //System.out.println("For " + row.size() + " entries");
                            if(!isReading)
                            {
                                System.out.println("not reading anymore");
                                return;
                            }
                            
                            try {
                                //URL aaa = new URL(MainController.API_ROOT + MainController.API_CHECKIN_URL);
                                String checkin_api_url = MainController.API_ROOT + MainController.API_CHECKIN_URL;
                                
                                String epc_string = "";
                                for (int i = 0; i < row.size(); i++){
                                    TagResults tr = row.get(i);
                                    
                                    epc_string += tr.getEpc();
                                    if (i < row.size()-1) epc_string += ",";
                                    
                                  
                                }
                                
                                    //System.out.println("Trying to send API call: " + checkin_api_url);
                                    //System.out.println("Sending epc parameter: " + epc_string);
                                    HttpClient httpclient = HttpClients.createDefault();
                                    HttpPost httppost = new HttpPost(checkin_api_url);
                                    List<NameValuePair> params = new ArrayList<NameValuePair>(2);
                                    params.add(new BasicNameValuePair("epc", epc_string));
                                    ...
                                    if (entity != null) {
                                        ...
                                        try {
                                            ...
                                            if (stream_content.length() < 1){
                                                ...
                                                showWarningErrorMessage("success", stream_content);
                                            }
                                        ...
                                        showWarningErrorMessage("error", "Error reading API call. Data was malformed.");
                                    }
                    ...
                    Platform.runLater(new Runnable()
                    {
                        ...
                        public void run()
                        {  
                            //System.out.println("run later platform thig");
                            ...
                            assignEpcTableView.getItems().clear();
                            assignEpcTableView.setItems(row);
                        }
                    });
                    Thread.sleep(2000);
                }
    ...
    public void assignTableViewConfiguration(){
        //set them to the epc one, because that seems right?...
        memberIDColumn.setCellValueFactory(new PropertyValueFactory<MemberResult, String>("memberID"));
        memberNameColumn.setCellValueFactory(new PropertyValueFactory<MemberResult, String>("memberName"));
        memberAssignColumn.setCellValueFactory(new PropertyValueFactory<MemberResult, Button>("memberAssign"));
        memberEpcColumn.setCellValueFactory(new PropertyValueFactory<MemberResult, String>("memberEPC"));
        
        //assignTableView.widthProperty().divide(5);
        memberIDColumn.setVisible(true);
        memberNameColumn.setVisible(true);
        memberAssignColumn.setVisible(true);
        memberEpcColumn.setVisible(true);
        
        //assign epc table
        assignEpcColumn.setCellValueFactory(new PropertyValueFactory<TagResults, String>("epcButton"));
    ...
    private Runnable updateFirmwareThread = new Runnable()
    {
        ...
        public void run()
        {
            ...
            {
                setTitledPanesStatus(false, false, false, false, false,false);
                setTitledPanesExpandedStatus(false, false, false, false, false, true);
   ...
   public void stopFirmwareProgress()
    ...
    public void updateAssignTable(){
        //System.out.println("Get Assign Table");
        assignTableView.getItems().clear();
        assignEpcTableView.getItems().clear();
        uniqueMemberCount.setText("0");
        ...
        try {
            //URL aaa = new URL(MainController.API_ROOT + MainController.API_CHECKIN_URL);
            String checkin_roster_api_url = MainController.API_ROOT + MainController.API_UPDATE_ROSTER_URL;
            
         
            //System.out.println("Trying to send API call: " + checkin_roster_api_url);
            HttpClient httpclient = HttpClients.createDefault();
            HttpPost httppost = new HttpPost(checkin_roster_api_url);
            List<NameValuePair> params = new ArrayList<NameValuePair>(2);
            params.add(new BasicNameValuePair("secret_ley", MainController.API_SECRET_KEY));
            httppost.setEntity(new UrlEncodedFormEntity(params, "UTF-8"));
            ...
            if (entity != null) {
                ...
                try {
                    ...
                    if (stream_content.length() < 1){
                        ...
                        //showWarningErrorMessage("success", stream_content);
                        JSONArray json_array = new JSONArray(stream_content);
                        int json_array_length = json_array.length();
                        uniqueMemberCount.setText(""+json_array_length);
                        assign_row = FXCollections.observableArrayList();
                        for (int i = 0; i < json_array_length; i++){
                            JSONObject member_1 = json_array.getJSONObject(i);
                        
                            System.out.println(member_1.toString());

                            String member_name = member_1.getString("name");
                            int member_id = member_1.getInt("id");
                            String member_epc = member_1.getString("epc");
                            //System.out.println(member_name + ": " + member_id);
                            MemberResult mr = new MemberResult(member_name, member_id, member_epc);
                            assign_row.add(mr);
                            
                            
                        }
                        assignTableView.getItems().clear();
                        assignTableView.setItems(assign_row);
    ...
    public void manualRead() {
        //System.out.println("Starting to manual read"); 
        readListener = new PrintListener();
        exceptionListener = new TagReadExceptionReceiver();
        statsListener = new ReaderStatsListener();
        r.addStatsListener(statsListener);
        ...
        /*
        try {
           TagReadData[] trd = r.read(1000);
           System.out.println("Size: " + trd.length);
           uniqueTagCount.setText(""+trd.length);
            for(int i = 0; i < trd.length; i++){
                readListener.tagRead(r, trd[i]);
                //epcColumn.setText(trd[i].epcString());
            } 
        }
        catch (Exception e){
   ...
   private void revertToDefaultSettings(ActionEvent event)
   {
       try
       {
           ...
           SerialReader srdr = (SerialReader)r;
           srdr.cmdBootBootloader();
           srdr.cmdBootFirmware();
